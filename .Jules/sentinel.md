...
## 2023-10-25 - Prevent Insecure Configuration File Writes
**Vulnerability:** Configuration files written with standard `fs::write` or incomplete `OpenOptions` lacked proper permissions (`0600`), potentially making them world-readable on Unix and lacking secure ACLs on Windows.
**Learning:** Fallback mechanisms that bypass centralized secure file utilities (like `crate::utils::write_secure`) often introduce subtle permission vulnerabilities and TOCTOU bugs, especially when existing files are not explicitly chmod'd.
**Prevention:** Always use centralized secure writing utilities (`write_secure`) unconditionally for configuration and sensitive data. Remove insecure fallbacks and avoid writing custom `fs::OpenOptions` blocks without applying `set_permissions` explicitly.
